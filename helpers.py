...
def reqcheck(password):
    """Check if a password satisfies the requirement"""
    # comment out the line below to disable password requirement check
    # return True
    ...
    number = False
    for c in password:
        if not lower and c in string.ascii_lowercase:
            lower = True
            continue
        if not upper and c in string.ascii_uppercase:
            upper = True
            continue
        if not number and c in string.digits:
            number = True
            continue
    if not (lower or upper or number):
        return False
    return True
